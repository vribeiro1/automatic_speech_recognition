...
from text_preprocessing import BLANK, text_processor


def collate_fn(batch):
    filepaths = [item[0] for item in batch]

    melspecs = [item[1].T for item in batch]
    ...
    padded_melspecs = padded_melspecs[:, melspecs_sort_indices, :]
    ...
    padded_transcriptions = padded_transcriptions.T
...
class LibriSpeechDataset(Dataset):
    def __init__(self, datadir, blank=BLANK, sample_rate=16000, n_fft=1024, win_length=1024, hop_length=256, n_mels=80, f_min=0, f_max=None):
        super().__init__()
        ...
        self.vocabulary = self.make_vocabulary(self.df.transcription, blank)
    ...
    @staticmethod
    def make_vocabulary(transcriptions, blank):
        all_tokens = set(funcy.flatten(map(list, transcriptions)))
        vocabulary = {blank: 0}
        vocabulary.update({token: i for i, token in enumerate(all_tokens, start=1)})
        return vocabulary
    ...
    def __getitem__(self, index):
        ...
        melspec = dynamic_range_compression(self.melspectrogram(audio_mono))

        tokens = torch.tensor(text_processor(transcription, self.vocabulary), dtype=torch.long)
